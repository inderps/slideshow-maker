var SlideShow = {
    ...
    renderElement: function(data, id){
        ...
        $("#" + id + "hnd").css({left: startLeft + 'px', top: startTop + 'px'}).delay(this.elementDelay).animate({left: data.left+'px', top: data.top+'px'}, 700).animate({left: startLeft + 'px', top: startTop + 'px'}, 700, function() {
            $("#" + id + "hnd").remove();
        });
//        $("#" + id + "hnd").delay(this.elementDelay + 100).remove()
